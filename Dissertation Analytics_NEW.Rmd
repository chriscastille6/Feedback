...
# Fix the [PA,NA] -> GS effects to zero since they are non-significant.
DissSEM3 <- '
  # latent variable definitions - Bifactor added
    POFA   =~ PercepAcc1 + PercepAcc2 + PercepAcc3 + PercepAcc4Reco + PercepAcc5
    PA =~ Interested + Excited + Strong + Enthusiastic + Proud + Determined + Attentive + Active + Inspired + Alert
...
fitCorResiduals <- resid(fitc, type = 'cor')$cov
...
# multiple regression
model <- lm(POFA ~ FdbkValN + FdbkTypeN + FdbkAccN + FdbkTypeN*FdbkAccN, AccTy)
summary(model)
```
