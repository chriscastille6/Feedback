...
'
fit <- lavaan::cfa(DissCFA1, data=likert, orthogonal=FALSE)
MI <- modindices(fit)
...
#sort FeedbackAccuracy for graphing purposes
dat2$FeedbackAccuracy <- factor(dat2$FeedbackAccuracy, levels = c("inaccurate","accurate"))
plot1 <- ggplot(dat2, aes(x = FeedbackAccuracy, y = mean, fill = FeedbackType))+
  geom_bar(stat='identity', position=dodge)+
  geom_errorbar(limits, position=dodge, width=0.25)+
  apatheme+
  ylab('Mean POFA')+
...
'
#Fit 'SEM' model to data
fit <- lavaan::sem(DissSEM3, data , std.lv = TRUE, estimator = "DWLS", se = "bootstrap", bootstrap = 10000)
summary(fit, fit.measures=TRUE,standardized = TRUE, rsquare=TRUE, ci=TRUE) 
```

```{SEM for OBHDP Paper}
#SEM - Desire to Respond and Justice removed
DissSEM3 <- '
  # latent variable definitions - REMOVED JITTERY AND IRRITABLE
    POFA   =~ PercepAcc1 + PercepAcc2 + PercepAcc3 + PercepAcc4Reco + PercepAcc5
    PA =~ Interested + Excited + Strong + Enthusiastic + Proud + Determined + Attentive + Active + Inspired + Alert
...
#Total Significant Effects
  #TotalAccEff := f11*e11*c11*d21*c21*a32 + f11*e11*d31*c21*a32
  TotalAccEff := f11*e11*c11*d21*c21 + f11*e11*d31*c21
'
#Fit 'SEM' model to data
fit <- lavaan::sem(DissSEM3, data , std.lv = TRUE, estimator = "DWLS", se = "bootstrap", bootstrap = 10000)
summary(fit, fit.measures=TRUE,standardized = TRUE, rsquare=TRUE, ci=TRUE) 
...
#Process followed from nickmachalak.com/blog_entries/2018/nrg02/nrg02.html
```{r Conditional Indirect Effects Analysis}
##Add a column in both datasets that idenitifies FIT group
##Bind Nominal and Relative data by rows
##Mean-center predictors
##Compute interaction terms
###Print first and last 5 observations
###Save to data folder
